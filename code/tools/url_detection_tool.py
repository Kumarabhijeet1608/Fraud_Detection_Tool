...
def load_url_model():
    with open("tools\\urldetec11.pkl", "rb") as file:
...
def predict_sentiment(review, grid_search, vectorizer):
    ...
    return grid_search.predict(review_vectorized)[0]
...
def print_feature_names(features):
...
def combine_predictions(url, review):
    # Load models
    ...
    # Predict URL safety
    url_prediction = url_model.predict(features)[0]

    # Process review and predict sentiment
    sentiment_prediction = predict_sentiment(review, sentiment_model, vectorizer)

    # Analyze named entities
    entities = analyze_entities(review)

    # Combine results
    url_weighted_score = 0.40 if url_prediction == 1 else 0.175 if url_prediction == 0 else 0.0
    sentiment_weighted_score = 0.30 if sentiment_prediction == "positive" else 0.0
    ner_weighted_score = 0.30 if entities else 0.0

    final_score = url_weighted_score + sentiment_weighted_score + ner_weighted_score

    if final_score >= 0.5:
        final_result = "Overall, the URL and the content are likely safe."
    else:
        final_result = "Overall, the URL or the content may be risky."
    
    # Print feature names
    print_feature_names(features)
    return {"final_result": final_result, 
            # "url_prediction":str(url_prediction),
              "sentiment_prediction":sentiment_prediction,
            #  "entities": entities,
            #  "features":features
            }

def open_particular_file(folder_name):
    # Create the full path to the result folder
    base_folder = "result"
    folder_path = os.path.join(base_folder, folder_name)
    
    # Construct the path for the example.com.txt file
    file_path = os.path.join(folder_path, "scrape.txt")
    ...
    if os.path.exists(file_path):
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            content = file.read()
            return(content)  # Print or return content
    else:
        return(False)

def fraud_detection(url, folder):
    data = open_particular_file(folder)
    if data is False:
        return "Detection cannot be performed"
    return combine_predictions(url, data)